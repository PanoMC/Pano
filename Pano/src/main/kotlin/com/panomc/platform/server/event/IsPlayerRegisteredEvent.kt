...
) : ServerEvent<IsPlayerRegisteredEventRequest, IsPlayerRegisteredEventResponse>() {
    override suspend fun handle(request: IsPlayerRegisteredEventRequest, server: Server): IsPlayerRegisteredEventResponse {
        val sqlClient = databaseManager.getSqlClient()

        val registered = databaseManager.userDao.existsByUsername(request.username, sqlClient)

        return IsPlayerRegisteredEventResponse(registered)
    }
}